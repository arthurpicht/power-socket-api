...
public class StatusParser {
    ...
    public static Status parseStatusHtml(String deviceId, String html, OutletIds outletIds) throws PowerSocketApiException {
        ...
        List<Status.OutletStatus> outletStatusList = parseLine(statusLine, outletIds);
        return new Status(deviceId, outletStatusList);
    }

    public static Status parseActionHtml(String deviceId, String html, OutletIds outletIds) throws PowerSocketApiException {
        List<String> htmlLines = StringHelper.splitByLineBreaks(html);
    ...
    public static List<Status.OutletStatus> parseLine(String string, OutletIds outletIds) {
        String[] outlets = string.split("<br>");
        if (outlets.length != 8)
            throw new IllegalStateException("Exactly 8 outlet status infos expected.");
        List<String> outletIdList = outletIds.getIds();
        List<Status.OutletStatus> outletStatusList = new ArrayList<>();
        for (int i = 0; i<8; i++) {
            String outletId = outletIdList.get(i);
            ...
            Status.OutletStatus outletStatus = new Status.OutletStatus(outletId, outletName, power);
